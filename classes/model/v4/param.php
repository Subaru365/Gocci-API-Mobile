...
class Model_V4_Param extends Model
{
    ...
    public function setGlobalCode_ERROR_SESSION_EXPIRED($payload = '')
    {
    ...
    public function setGlobalCode_ERROR_CLIENT_OUTDATED($payload = '')
    {
    ...
    public function set_auth_login_ERROR_IDENTITY_ID_NOT_REGISTERD($payload = '')
    {
    ...
    public function set_auth_signup_ERROR_USERNAME_ALREADY_REGISTERD($payload = '')
    {
    ...
    public function set_auth_password_ERROR_USERNAME_NOT_REGISTERD($payload = '')
    {
    ...
    public function set_auth_password_ERROR_PASSWORD_NOT_REGISTERD($payload = '')
    {
    ...
    public function set_auth_password_ERROR_PASSWORD_WRONG($payload = '')
    {
    ...
    public function set_set_sns_link_ERROR_SNS_PROVIDER_TOKEN_NOT_VALID($payload = '')
    {
    ...
    public function set_set_sns_link_ERROR_PROVIDER_UNREACHABLE($payload = '')
    {
    ...
    public function set_unset_sns_link_ERROR_SNS_PROVIDER_TOKEN_NOT_VALID($payload = '')
    {
        $this->status['code']    = 'ERROR_SNS_PROVIDER_TOKEN_NOT_VALID';
        $this->status['message'] = "The provided sns token is invalid or has expired";
        if (!empty($payload)) {
    ...
    public function set_unset_sns_link_ERROR_PROVIDER_UNREACHABLE($payload = '')
    {
        $this->status['code']    = 'ERROR_PROVIDER_UNREACHABLE';
        $this->status['message'] = "The providers server infrastructure appears to be down";
        if (!empty($payload)) {
    ...
    private function setReqParams_set_comment($input_params)
    {
        ...
        if(!empty($input_params['re_user_id'])) {
            ...
            if(preg_match('/^[0-9,]{1,9}$/', $input_params['re_user_id'])) {
                ...
                $this->status['message'] = "Parameter 're_user_id' is malformed. Should correspond to '^[0-9,]{1,9}$'";
            }
    ...
    private function setReqParams_unset_comment($input_params)
    {
        if(!empty($input_params['comment_id'])) {
            ...
            if(preg_match('/^\d{1,9}$/', $input_params['comment_id'])) {
                ...
                $this->status['code']    = 'ERROR_PARAMETER_COMMENT_ID_MALFORMED';
                $this->status['message'] = "Parameter 'comment_id' is malformed. Should correspond to '^\d{1,9}$'";
            }
        
        } else {
            $this->status['code']    = 'ERROR_PARAMETER_COMMENT_ID_MISSING';
    ...
    private function setReqParams_set_comment_edit($input_params)
    {
        if(!empty($input_params['comment_id'])) {
    ...
    public function set_get_post_ERROR_POST_DOES_NOT_EXIST($payload = '')
    {
        $this->status['code']    = 'ERROR_POST_DOES_NOT_EXIST';
        $this->status['message'] = "The post with the provided ID does not exist";
        if (!empty($payload)) {
            $this->status['payload'] = $payload;
        }
    }
    ...
    public function set_get_post_ERROR_POST_WAS_NEVER_COMPLETED($payload = '')
    {
        $this->status['code']    = 'ERROR_POST_WAS_NEVER_COMPLETED';
        $this->status['message'] = "The video file was never uplaoded to AWS S3";
        if (!empty($payload)) {
            $this->status['payload'] = $payload;
        }
    }
    ...
    private function setReqParams_set_post($input_params)
    {
        ...
        if(!empty($input_params['movie_name'])) {
            ...
            if(preg_match('/^\d{4}(-\d{2}){5}_\d{1,9}$/', $input_params['movie_name'])) {
                ...
                $this->status['message'] = "Parameter 'movie_name' is malformed. Should correspond to '^\d{4}(-\d{2}){5}_\d{1,9}$'";
            }
        ...
        if(!empty($input_params['value'])) {
            ...
            if(preg_match('/^\d{1,9}$/', $input_params['value'])) {
                $this->req_params['value'] = $input_params['value'];
        
            } else {
                $this->status['code']    = 'ERROR_PARAMETER_VALUE_MALFORMED';
                $this->status['message'] = "Parameter 'value' is malformed. Should correspond to '^\d{1,9}$'";
            }
        ...
        if(!empty($input_params['memo'])) {
            ...
            if(preg_match('/^(\n|[^\p{Cc}]){1,1000}$/u', $input_params['memo'])) {
        ...
        if(!empty($input_params['cheer_flag'])) {
            ...
            if(preg_match('/^1|0$/', $input_params['cheer_flag'])) {
                $this->req_params['cheer_flag'] = $input_params['cheer_flag'];
        
            } else {
                $this->status['code']    = 'ERROR_PARAMETER_CHEER_FLAG_MALFORMED';
                $this->status['message'] = "Parameter 'cheer_flag' is malformed. Should correspond to '^1|0$'";
            }
    ...
    private function setReqParams_set_post_crash($input_params)
    {
        ...
        if(!empty($input_params['address'])) {
            ...
            if(preg_match('/^[^\p{Cc}]{1,100}$/u', $input_params['address'])) {
                $this->req_params['address'] = $input_params['address'];
        
            } else {
                $this->status['code']    = 'ERROR_PARAMETER_ADDRESS_MALFORMED';
                $this->status['message'] = "Parameter 'address' is malformed. Should correspond to '^[^\p{Cc}]{1,100}$'";
            }
        
        } else {
            $this->status['code']    = 'ERROR_PARAMETER_ADDRESS_MISSING';
            $this->status['message'] = "Parameter 'address' does not exist.";
        }
    ...
    private function setReqParams_set_memo_edit($input_params)
    {
        if(!empty($input_params['post_id'])) {
        ...
        if(!empty($input_params['memo'])) {
            ...
            $this->status['code']    = 'ERROR_PARAMETER_MEMO_MISSING';
            $this->status['message'] = "Parameter 'memo' does not exist.";
        }
    ...
    public function set_set_username_ERROR_USERNAME_ALREADY_REGISTERD($payload = '')
    {
        $this->status['code']    = 'ERROR_USERNAME_ALREADY_REGISTERD';
        $this->status['message'] = "The provided username was already registerd by another user";
        if (!empty($payload)) {
    ...
    private function setReqParams_get_username($input_params)
    {
        if(!empty($input_params['username'])) {
    ...
    private function setReqParams_get_follower_rank($input_params)
    {
        if(!empty($input_params['page'])) {
